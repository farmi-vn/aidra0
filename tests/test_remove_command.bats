...
@test "--remove command handles non-existent container with error" {
    ...
    export MOCK_CONTAINER_NAME="aidra0-nonexistent"
    export BATS_TEST_MODE=1
    ...
    run bash -c '
        ...
        # Simulate remove_container function logic
        container_name="aidra0-nonexistent"
...
@test "--remove command generates correct container name" {
    ...
    run bash -c '
        ...
        base_name="my-app"
        container_name="aidra0-$base_name"
        echo "generated_container_name: $container_name"
    '

    [[ "$output" == *"generated_container_name: aidra0-my-app"* ]]
    [ "$status" -eq 0 ]
}
...
@test "--remove command shows available containers on error" {
    ...
    run bash -c '
        ...
        if ! docker ps -a --format "{{.Names}}" | grep -q "^$container_name$"; then
            ...
            echo "aidra0 containers:"
            echo ""
            echo "NAME                    STATUS                  MOUNTED DIRECTORY"
            echo "To remove a container: aidra0 --remove <name>"
            exit 1
        fi
    '
    ...
    [[ "$output" == *"To remove a container: aidra0 --remove <name>"* ]]
    [ "$status" -eq 1 ]
}
