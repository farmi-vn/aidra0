...
# Makefile for aidra0 version management
...
.PHONY: help
help:
	@echo "$(BLUE)aidra0 Version Management$(RESET)"
	@echo ""
...
.PHONY: install
install:
	@echo "$(BLUE)Installing aidra0 to /usr/local/bin...$(RESET)"
	@if [ ! -w "/usr/local/bin" ]; then \
		echo "$(RED)Error: No write permission to /usr/local/bin$(RESET)"; \
		echo "Try: sudo make install"; \
		exit 1; \
	fi
	@cp $(SCRIPT_FILE) /usr/local/bin/aidra0
	@chmod +x /usr/local/bin/aidra0
	@echo "$(GREEN)✓ Installed aidra0 to /usr/local/bin$(RESET)"

# Remove script from /usr/local/bin
.PHONY: uninstall
uninstall:
	@echo "$(BLUE)Removing aidra0 from /usr/local/bin...$(RESET)"
	@if [ ! -w "/usr/local/bin" ]; then \
		echo "$(RED)Error: No write permission to /usr/local/bin$(RESET)"; \
		echo "Try: sudo make uninstall"; \
		exit 1; \
	fi
	@rm -f /usr/local/bin/aidra0
	@echo "$(GREEN)✓ Removed aidra0 from /usr/local/bin$(RESET)"
